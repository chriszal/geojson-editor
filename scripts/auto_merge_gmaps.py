...
def main():
    ...
    for g in gmaps_beaches:
        ...
        # Find closest match in current.json that has a similar name
        best_match = None
        min_dist = float("inf")
        ...
        for feat in fc["features"]:
            ...
            if dist <= args.max_distance:
                # Check names
                c_names = feat["properties"].get("name", [])
                if not isinstance(c_names, list):
                    c_names = [c_names]
                
                # Check if any of the existing beach names match the gmaps name
                any_match = False
                for c_name in c_names:
                    if names_are_similar(c_name, g_name):
                        any_match = True
                        break
                        
                if any_match and dist < min_dist:
                    min_dist = dist
                    best_match = feat
                    
        if best_match:
            c_name = best_match["properties"].get("name", ["Unnamed"])[0]
            # Safe print names
            safe_g_name = g_name.encode('ascii', errors='replace').decode('ascii')
            safe_c_name = c_name.encode('ascii', errors='replace').decode('ascii')
            print(f"Match: Merge '{safe_g_name}' -> '{safe_c_name}' (distance: {min_dist:.1f}m)")
